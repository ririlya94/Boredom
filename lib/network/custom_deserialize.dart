class CustomDeserialize<T> {
  ...
  CustomDeserialize(dynamic json, {
    String key = '',
    T Function(
        Map<String, dynamic>, {
    ...
    final dynamic j = key.isEmpty ? json : json[key];
    if (j == null) {
      missingKeys.add(key);
    }
  ...
  bool _isValid(dynamic json,
      List<String> requiredFields,
      List<String> missingKeys,) {
    if (json == null) return false;
    if (requiredFields.isEmpty) return true;
    final List<String> invalid = requiredFields
        .where((e) => e
        .trim()
        .isNotEmpty && json[e] == null)
        .toList();
    if (invalid.isNotEmpty) {
      for (final String s in invalid) {
